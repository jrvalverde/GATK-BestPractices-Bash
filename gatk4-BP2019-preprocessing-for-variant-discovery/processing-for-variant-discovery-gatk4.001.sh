...
function PreProcessingForVariantDiscovery_GATK4() {
    ...
    if [ "$BY_SEQ_GRPS" == 'YES' ] ; then
        ...
        exit
        # This is useful for whole genome sequencing to parallelize
        # or split the work in pieces (e.g. by chromosome)
        # In our case, we can just do it all at once
        ...
        if [ ! -s exome.intervals.list ] ; then
            cp EXOME.intervals.list exome.intervals.list
        fi
        # Generate the recalibration model by interval
        $CALL BaseRecalibrator $sorted_all_bam exome.intervals.list
        for sorted_bam in $align_bwa_dir/*.duplicate_marked.sorted.bam ; do
            $CALL BaseRecalibrator $sorted_bam exome.intervals.list
        done
    fi
