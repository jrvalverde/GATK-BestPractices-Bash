...
function SamToFastqAndBwaMem () {
    ...
    output_unmerged_bam=$align_dir/$bam_basename.unmerged.bam
...
function MergeBamAlignment () {
    ...
    bash_ref_fasta=${ref_fasta}
    bam_basename=`basename $unmapped_bam $unmapped_bam_suffix`
    unmerged_aligned_bam=$align_dir/$bam_basename.unmerged.bam
    output_bam=$align_dir/${bam_basename}.bam
...
function MarkDuplicates() {
    ...
    output_bam=$align_dir/$output_bam_basename.aligned.unsorted.duplicates_marked.bam
...
function SortAndFixTags() {
    ...
    output_bam=$align_dir/${unsorted_bam_base}.aligned.duplicate_marked.sorted.bam
    ...
    if [ ! -s $output_bam ] ; then
        ...
        # if that failed, try with SAMTOOLS (which is faster BTW)
        if [ ! -s sorted.bam ] ; then
    ...
    echo "Created $output_bam"  ; rm sorted.bam
}
...
function BaseRecalibrator() {
    ...
    recalibration_report_filename=$align_dir/${base_out_name}.recal_data.csv
...
function GatherBqsrReports {
    ...
    output_report_filename=$align_dir/${base_file_name}.all.recal_data.csv
...
function ApplyBQSR() {
    ...
    output_bam=$align_dir/$base_name.aligned.duplicates_marked.recalibrated.bam
...
function GatherBamFiles() {
    ...
    output_bam=./$align/$base_file_name.all.align.dup.recal.bam
...
function PreProcessingForVariantDiscovery_GATK4() {
    ...
    merged_bams=$( ls $align_dir/*001.bam ) 
    ...
    marked_all_bam=$align_dir/${base_file_name}.aligned.unsorted.duplicates_marked.bam 



    # Sort deduped BAM files and fix tags
    # This will also consider the aggregate file
    for i in $align_dir/*.aligned.unsorted.duplicates_marked.bam ; do
        unsorted_bam_base=`basename $i .aligned.unsorted.duplicates_marked.bam`
        sorted_bam=$align_dir/$unsorted_bam_base.aligned.duplicate_marked.sorted.bam
        if [ ! -e $sorted_bam ] ; then
            $CALL SortAndFixTags $i
        fi
    done
    # the sorted-fixed aggregate bam file
    sorted_all_bam=$align_dir/${base_file_name}.aligned.duplicate_marked.sorted.bam
    ...
    if [ ! -e EXOME.intervals.list ] ; then
        ...
        echo "e.g. ln -s ../panel/INGEM_panel5_120_mod_interval_format.intervals EXOME.intervals.list"
        exit
    fi
    ...
    individual_reports=$( ls $align_dir/*.recal_data.csv | grep -v $base_file_name )
    
#    $CALL GatherBqsrReports "${individual_reports[@]}"
    output_bqsr_report=$align_dir/${base_file_name}.all.recal_data.csv
	...
	# DO NOT USE: THE FUNCTION AND LOGIC ARE INCOMPLETE
        cat sequence_grouping_with_unmapped.txt | while read subgroup ; do
            ...
            recal_all_bam=$align_dir/$base_file_name.aligned.duplicates_marked.recalibrated.bam
            for sorted_bam in $align_dir/*.duplicate_marked.sorted.bam ; do
    ...
    # a separate BAM
    individual_bams=$( ls $align_dir/*recalibrated.bam | grep -v $base_file_name )
    
    # Fails with error indexing a read aligned to chr2 (should be 17?)
    ###$CALL GatherBamFiles "${individual_bams[@]}"
    #output_bam=./$align_dir/$base_file_name.all.align.dup.recal.bam
}
