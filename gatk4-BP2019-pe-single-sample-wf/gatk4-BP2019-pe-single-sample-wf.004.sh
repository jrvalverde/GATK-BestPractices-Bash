...
# whether to use CNN variant filtration and which type of it
cnn_type=""
#cnn_type="CNN_1D"
#cnn_type="CNN_2D"	# Using this one requires re-running HaplotypeCaller
...
#  scatter (index in range(ScatterIntervalList.interval_count)) {
    ...
    if [ "$ANNOTATE" == "NO" ] ; then
        if [ ! -e $gvcf_dir/`basename $gvcf` ] ; then
            ...
            ln -sf `relpath g.vcf $gvcf` $gvcf_dir/
            ln -sf `relpath g.vcf $gvcf.tbi` $gvcf_dir/
        fi
    fi
...
#      output_vcf_name = final_gvcf_base_name + ".g.vcf.gz",
    ...
    if [ "$ANNOTATE" == "YES" ] ; then
        ...
        if [ ! -e $gvcf_dir/`basename $gvcf_ann` ] ; then
            if [ ! -d $gvcf_dir ] ; then mkdir -p $gvcf_dir ; fi
            ln -sf `relpath g.vcf $gvcf_ann` $gvcf_dir/`basename $gvcf_ann`
            ln -sf `relpath g.vcf $gvcf_ann.idx` $gvcf_dir/`basename $gvcf_ann.idx`
        fi
    fi
  ...
  #  }
    ...
    # By working over the annotated gvcf file we expect that the annotation
    # will be carried through filtering

    # 1. Hard filtering of the whole GVCF file (both SNPs and indels
    HardFilterVcf $gvcf $study_interval_list
    gvcf_filtered=${gvcf%.vcf*}.hard.filtered.vcf.gz

    # 2. CNNscore filtration
    if [ "$cnn_type" != "" ] ; then
        CNNScoreVariants $gvcf_filtered $hc_bamout
        gvcf_scored=${gvcf_filtered%.vcf*}.cnnscored.vcf

        FilterVariantTranches $gvcf_scored
        gvcf_filtered=${gvcf_scored%.vcf*}.filtered.vcf.gz
    fi

    # 3. Stringent LPZ hard filtering
    HardFilterVcf_LPZ $gvcf_filtered $study_interval_list
    gvcf_filtered=${gvcf_filtered%.vcf*}.lpz.vcf.gz

    if [ "$separate_SNP_INDEL" == "YES" ] ; then
    	# Right now the extraction of SNPs and INDELs does not work
        # so we will not use the following for the time being.
        
        # 3. Separate SNPs and Indels
        ExtractSNPs $gvcf_filtered
        snps_raw=${gvcf_filtered%.vcf*}.snps.vcf

        ExtractInDels $gvcf_filtered
        indels_raw=${gvcf_filtered%.vcf*}.indels.vcf

        # 4. Hard Filter SNPs and InDels
        HardFilterSNPs $snps_raw
        snps_filtered=${snps_raw%.vcf*}.filtered.vcf

        HardFilterInDels $indels_raw
        indels_filtered=${indels_raw%.vcf*}.filtered.vcf

        HardFilterVcf_LPZ $indels_filtered $study_interval_list
        indels_filtered_lpz=${indels_filtered%.vcf*}.lpz.vcf

        # And re-collect variant calling metrics
        CollectGvcfCallingMetrics $snps_filtered $study_interval_list
        CollectGvcfCallingMetrics $indels_filtered $study_interval_list
    fi
    CollectGvcfCallingMetrics $gvcf_filtered $study_interval_list
...
## HardFilterVcf()
# Usage:
#	HardFilterVcf input_vcf interval_list
#
# Description:
#
#
...
# Released under a LGPL or EU-LGPL license
#
function HardFilterVcf() {
  if [ $# -ne 2 ] ; then 
      echo "Err: ${FUNCNAME[0]} input_vcf interval_list" ; exit 1 
  else echo -e "\n>>> ${FUNCNAME[0]}" $* ; fi
    # File input_vcf
    ...
    input_vcf=$1
    interval_list=$2
    vcf_basename=${input_vcf%.vcf*}
    output_vcf_name=${vcf_basename}.hard.filtered.vcf.gz
   
    if [ ! -e $output_vcf_name ] ; then
        $gatk_exec --java-options "-Xms3000m" \
            VariantFiltration \
            -V ${input_vcf} \
            -L ${interval_list} \
...
## CNNScoreVariants()
# Usage:
#	CNNScoreVariants input_vcf [bamout]
#
# Description:
#
#
...
# Released under a LGPL or EU-LGPL license
#
function CNNScoreVariants() {
    if [ $# -gt 2 ] ; then
        echo ">>> ${FUNCNAME[0]} input_vcf [bamout]"
        return
    elif [ $# -eq 2 ] ; then
        # can use a bamout file produced by HaplotypeCaller that has been
        # subsequently sorted with SortSam(), hence it will be named 
        # *.bamout.sorted.bam
        bamout=$2
    else
        echo ">>> ${FUNCNAME[0]} $*"
    ...
    # global File ref_fasta
    # global File ref_dict
    output_vcf=${input_vcf%.vcf*}.cnnscored.vcf
    
  #    For CNNScoreVariants to run with a 2D model, a bamout must 
  # be created by HaplotypeCaller. The bamout is a bam containing information 
  # on how HaplotypeCaller remapped reads while it was calling variants. See 
  # https://gatkforums.broadinstitute.org/gatk/discussion/5484/howto-generate-a-bamout-file-showing-how-haplotypecaller-has-remapped-sequence-reads for more details.
  # NOTE that you should call SortSam $bamout afterwards.
    if [ "$bamout" == "" ] ; then
        bamout_opt=""
        tensort_opt="-tensor-type reference"
        info_key="CNN_1D"
    else
        bamout_opt="-I $bamout"
        tensor_opt="-tensor-type read-tensor" 
	info_key="CNN_2D"
    fi       
...
# Usage:
#	FilterVariantTranches input_vcf
#
# Description:
#
#
...
# Released under a LGPL or EU-LGPL license
#
function FilterVariantTranches() {
  if [ $# -ne 1 ] ; then 
      echo "Err: ${FUNCNAME[0]} input_vcf " ; exit 1 
  else echo -e "\n>>> ${FUNCNAME[0]}" $* ; fi
...
#  }
    input_vcf=$1
    info_key=$cnn_type	# if the VCF was scored with CNN and bamout use CNN_2D
    vcf_basename=${input_vcf%.vcf*}


    if [ ! -e ${vcf_basename}.filtered.vcf.gz ] ; then
        $gatk_exec \
            --java-options -Xmx6g \
            FilterVariantTranches \
            -V ${input_vcf} \
            -O ${vcf_basename}.filtered.vcf.gz \
            --snp-tranche "100.0" --snp-tranche "99.95" --snp-tranche "99.9" --snp-tranche "99.8" --snp-tranche "99.6" --snp-tranche "99.5" --snp-tranche "99.4" --snp-tranche "99.3" --snp-tranche "99.0" --snp-tranche "98.0" --snp-tranche "97.0" --snp-tranche "90.0" \
            --indel-tranche "100.0" --indel-tranche "99.95" --indel-tranche "99.9" --indel-tranche "99.8" --indel-tranche "99.6" --indel-tranche "99.5" --indel-tranche "99.4" --indel-tranche "99.3" --indel-tranche "99.0" --indel-tranche "98.0" --indel-tranche "97.0" --indel-tranche "90.0" \
            --resource ${hapmap_vcf} \
            --resource ${omni_vcf} \
            --resource ${G1000_vcf} \
            --resource ${dbSNP_vcf} \
...
function ExtractSNPs() {
  if [ $# -ne 1 ] ; then 
    ...
    raw_variants=$1
    raw_snps=${raw_variants%.vcf*}.snps.vcf
...
# Released under a LGPL or EU-LGPL license
#
function ExtractInDels() {
  if [ $# -ne 1 ] ; then 
      echo "Err: ${FUNCNAME[0]} input_vcf" ; exit 1 
  else echo -e "\n>>> ${FUNCNAME[0]}" $* ; fi

    raw_variants=$1
    raw_indels=${raw_variants%.vcf*}.indels.vcf
    ...
    if [ ! -s $raw_indels ] ; then
        echo "Failed to make $raw_indels"
        exit
    fi
}
...
# Released under a LGPL or EU-LGPL license
#
function HardFilterSNPs() {
    if [ $# -ne 1 ] ; then 
        echo "Err: ${FUNCNAME[0]} input_vcf" ; exit 1 
    else echo -e "\n>>> ${FUNCNAME[0]}" $* ; fi

    raw_snps=$1
    filtered_snps=${raw_snps%.vcf*}.filtered.vcf
    ...
    if [ ! -s $filtered_snps ] ; then
        echo "Failed to make $filtered_snps"
        exit
    fi
...
# Released under a LGPL or EU-LGPL license
#
function HardFilterInDels() {
    if [ $# -ne 1 ] ; then 
        echo "Err: ${FUNCNAME[0]} input_vcf" ; exit 1 
    else echo -e "\n>>> ${FUNCNAME[0]}" $* ; fi

    raw_indels=$1
    filtered_indels=${raw_indels%.vcf*}.filtered.vcf
    ############################################################################
    # Filter Variants
    # 
    # The first step is designed to maximize sensitivity and is thus very lenient
    ...
    if [ ! -s $filtered_indels ] ; then
        echo "Failed to make $filtered_indels"
        exit
    fi
...
## HardFilterVcf_LPZ()
# Usage:
#	HardFilterVcf_LPZ input_vcf interval_list
#
# Description:
#	This sould apply a more stringent set of legacy filters
#
# (c) CNB-CSIC. 2019
# Released under a LGPL or EU-LGPL license
#
function HardFilterVcf_LPZ() {
    if [ $# -ne 2 ] ; then 
        echo "Err: ${FUNCNAME[0]} input_vcf interval_list" ; exit 1 
    else echo -e "\n>>> ${FUNCNAME[0]}" $* ; fi
    ...
    input_vcf=$1
    interval_list=$2
    filtered_lpz=${input_vcf%.vcf*}.lpz.vcf.gz
    ...
    if [ ! -s "$filtered_lpz" ] ; then
        echo "Failed to make $filtered_lpz"
        exit
    fi
}
...
function ValidateGVCF() {
  ...
  if [ ! -e ${input_vcf%.vcf*}.validated ] ; then
    ...
    ${gatk_exec} \
      ...
      --dbsnp ${dbSNP_vcf} \
      --warn-on-errors
    if [ $? -eq 0 ] ; then touch ${input_vcf%.vcf*}.validated ; fi
  fi
}
