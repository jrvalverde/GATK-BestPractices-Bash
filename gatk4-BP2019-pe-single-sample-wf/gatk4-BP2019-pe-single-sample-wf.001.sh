...
#  scatter (index in range(ScatterIntervalList.interval_count)) {
    ...
    if [ ! -e g.vcf/$gvcf ] ; then
        if [ ! -d g.vcf ] ; mkdir -p g.vcf ; done
        ln -s `relpath g.vcf $gvcf` g.vcf
    fi
...
#      output_vcf_name = final_gvcf_base_name + ".g.vcf.gz",
    ...
    gvcf_ann=${gvcf_ann/%.*/.ann.g.vcf}	# remove type (.vcf)
    if [ ! -e g.vcf.ann/$gvcf_ann ] ; then
        if [ ! -d g.vcf.ann ] ; mkdir -p g.vcf.ann ; done
        ln -s `relpath g.vcf $gvcf_ann` g.vcf.ann
    fi
...
function SamToFastqAndBwaMemAndMba() {
    ...
    if [ ! -s ${ref_alt} ]; then 
      echo "$ref_alt has zero size or does not exist"
      exit 1 
    fi
